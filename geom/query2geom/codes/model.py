...
def full_box_int(embeds1, embeds1_o, embeds2, embeds2_o):
    box1_int_box2 = box_int_centre(embeds1, embeds1_o, embeds2, embeds2_o)
    sign_vec = (embeds1 < embeds2)*2 - 1 #ie in {-1, 1}. -1 if embeds_1 < embeds_2= in that dim, else 1
    box1_int_box2_o = -sign_vec*box1_int_box2 + sign_vec*(embeds1 + sign_vec*embeds1_o)
    return box1_int_box2, box1_int_box2_o

def box_int_centre(embeds1, embeds1_o, embeds2, embeds2_o):
    '''
    This is the intersection of two boxes if they are a and b:
        C_c = C_c = ((C_a + O_a) + (C_b - O_b)) /2
    This assumes boc A is left of box B (ie A < B) in this dimenions
    so we need a way to ditinguish signs of O_a and O_b!
    '''
    sign_vec = (embeds1 < embeds2)*2 - 1 #ie in {-1, 1}. -1 if embeds_1 < embeds_2= in that dim, else 1
    return ((embeds1 + sign_vec*embeds1_o) + (embeds2 - sign_vec*embeds2_o)) / 2

def box_int_offset(embeds1, embeds1_o, embeds2, embeds2_o):
    '''
    given
        box1_int_box2 is the center of the intersection box
        b1 is box 1's center
        b2 is box 2's centre
    for i in dims(b1):
        if b1[i] < b2[i]:
            offset[i] = -box1_int_box2[i] + (b1[i] + b1_o[i])
                ... = 
        else:
            offset[i] = box1_int_box2[i] - (b1[i] - b1_o[i])
        or generally
            offset[i] = -sign_vec[i]*box1_int_box2[i]
                + sign_vec[i]*(b1[i] + sign_vec[i]*(b1_o[i])
    '''
    box1_int_box2 = box_int_centre(embeds1, embeds1_o, embeds2, embeds2_o)
    sign_vec = (embeds1 < embeds2)*2 - 1 #ie in {-1, 1}. -1 if embeds_1 < embeds_2= in that dim, else 1
    return -sign_vec*box1_int_box2 + sign_vec*(embeds1 + sign_vec*embeds1_o)

class MeanSet(nn.Module):
    '''
    It seems this is where they do their uniform average.
    I replaced this with out weighted average
    '''
    def __init__(self):
    ...
    def forward(self, embeds1, embeds1_o, embeds2, embeds2_o, embeds3 = [], embeds3_o=[]):
        '''
        So I think that this accepts either 2 or three boxes; if only 2, len(embeds3)
        will be 0. In any case:
            embedsN is the center of box N
            embedsN_o is the offset of box N
        '''
        if len(embeds3) > 0:
            '''
            Note that A n B n C = (A n B) n C
            '''
            # return torch.mean(torch.stack([embeds1, embeds2, embeds3], dim=0), dim=0)
            box1_int_box2, box1_int_box2_o = full_box_int(embeds1, embeds1_o, embeds2, embeds2_o)
            return box_int_centre(box1_int_box2, box1_int_box2_o, embeds3, embeds3_o)
        else:
            # return torch.mean(torch.stack([embeds1, embeds2], dim=0), dim=0)
            return box_int_centre(embeds1, embeds1_o, embeds2, embeds2_o)
            
class MinSet(nn.Module):
    '''
    This is where they take the min offsets of both boxes to get to the offset
    of the intersection box, it seems
    I replaced this with out geometric calculation of the offsets.
    '''
    def __init__(self):
    ...
    def forward(self, embeds1, embeds1_o, embeds2, embeds2_o, embeds3 = [], embeds3_o=[]):
        ...
        if len(embeds3) > 0:
            ...
            box1_int_box2, box1_int_box2_o = full_box_int(embeds1, embeds1_o, embeds2, embeds2_o)
            return box_int_offset(box1_int_box2, box1_int_box2_o, embeds3, embeds3_o)

        else:
            # return torch.min(torch.stack([embeds1, embeds2], dim=0), dim=0)[0]
            return box_int_offset(embeds1, embeds1_o, embeds2, embeds2_o)
